...
public class AutomaticProperty : SerializedProperty
{
    ...
    public AutomaticProperty(Scene scene, string name, PropertyInfo propertyInfo, object target) : base(name)
    {
        value = propertyInfo;
        this.target = target;
        this.scene = scene;
    ...
    public override void Edit()
    {
        ...
        current = Inspector.Inspect(scene, value.PropertyType, current, Name, out bool edited);
    ...
    public override void Load(Tag tag)
    {
        ...
        value.SetValue(target, NBTSerialization.Deserialize(tag, value.PropertyType, scene));
    }
}